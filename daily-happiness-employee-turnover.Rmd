---
title: "daily-happiness-employee-turnover"
author: "LC"
...
# Daily Happiness & Employee Turnover {.tabset}
...
A dataset found on [https://www.kaggle.com/harriken/employeeturnover](https://www.kaggle.com/harriken/employeeturnover). <br> 
  
This dataset is from the spanish company **myhappyforce**, an employee survey company. They send everyday the question *How happy ar you today at work?* to the employees, they then can answer with a vote from 1 to 4 and leave a comment. <br> 

For this case we will use two datasets:  

* *churn* which stores each employee and if this employee churned (if he left or has been fired)
* *votes* which stores every votes, who voted and the date this employee voted. <br>
...
> The main goal of this work will be to show a correlation between the happiness rate in a company and its turnover.
...
### Let's compute happiness rate by companies

```{r happinessRate, echo = TRUE}
happinessRate <- votes %>% 
  group_by(companyAlias) %>% 
...
Just an overview of the companies to show how a the average vote can differ from a company to another one. <br>
 <br>
```{r hrGraphs2}
...
```
 <br>
The "weekend effect" appears here, grades from friday to sunday are higher.  
Note however that the average number of votes on the weekend is lower than the rest of the week (*163* and *180* compared to *317* during workweek).  
 <br>
...
```  
 <br>
The happiness rate can also vary due to the season. As the graph above shows, the happiness rate vary during the year.  
 
## Employee Turnover

### Correlation between turnover and happiness rate
 <br>
 $turnover \ rate = 1 - \frac{Employees}{Employees \ + \ Churned}$  <br>
 <br>
According to LinkedIn, an average annual worldwide employee turnover rate is 10.9%. <br>

```{r employeeTurnover, echo = TRUE}
...
```
 <br>
We can spot a negative correlation between the vote and the turnover rate. <br>
 <br>
```{r etGraphs}
...
Thanks to this graph we can assume ther is a correlation between the daily average happiness vote and the turnover rate.  
 
## Raw code

```{r rawCode, eval = FALSE, echo = TRUE}
##Presentation
